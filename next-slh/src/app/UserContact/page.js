...
export default function UserContact() {
    ...
    const validateServices = (services) => {
        return services.length > 0 ? '' : '최소 하나의 서비스를 선택해주세요.';
    };
    ...
    const handleServiceChange = (serviceCode) => {
        setSelectedServices((prevSelectedServices) => {
            ...
            // Validate services and set error
    ...
    const handleSubmit = (event) => {
        ...
        .then(() => {
            ...
            setPhone('');
            setEmail('');
            setSelectedServices([]); // 선택한 서비스 초기화
            setBudget('');
            setSchedule('');
            setDescription('');
        })
    ...
    return (
        <section id="page04" className={`section ${inView ? 'view' : ''}`} ref={ref}>
            ...
            {/* 모바일 버전 */}
            <Link href="/UserContact/Detail" className="mob_btn x1000">문의하기</Link>
            <br />
            <br />
            <br />
            <br />
