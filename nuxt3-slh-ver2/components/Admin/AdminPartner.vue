...
const deletePartnerApi = "/api/partner"
...
const fetchData = async () => {
  try {
    loading.value = true
    const response = await axios.get(partnerApi)
...
const handleDelete = async (id) => {
  showConfirmDialog(
    ...
    async () => {
      try {
        dialog.value = false
        await axios.delete(`${deletePartnerApi}/${id}`)
        await fetchData()
          ...
          <tbody>
            <tr v-for="partner in displayedPartners" :key="partner.PARTNER_NB" class="partner-row">
              <td class="text-center">
                <v-img
                   v-if="partner.PARTNER_IMG && partner.PARTNER_IMG !== 'NO_IMG'"
                  ...
                  contain
                />
                <div v-else class="empty-img-placeholder"></div>
              </td>
              <td class="text-center">
