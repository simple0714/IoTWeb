  ...
  switch (selectedNav.value) {
    ...
    case 'contact':
      if (selectedContactId.value) {
        // console.log('Detail 컴포넌트 렌더링, ID:', selectedContactId.value)
        return h(AdminContactDetail, {
          contactId: selectedContactId.value,
          onBack: () => {
            // console.log('뒤로가기 실행')
            selectedContactId.value = null
      ...
      // console.log('Contact 컴포넌트 렌더링')
      return h(AdminContact, {
        onSelectContact: (id) => {
          // console.log('ID 선택됨:', id)
          selectedContactId.value = id
        }
      })
    default:
