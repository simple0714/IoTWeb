...
const findOneApi = "/api/about"
const updateApi = "/api/about/update"
...
onMounted(async () => {
  try {
    const response = await axios.get(`${findOneApi}/${router.currentRoute.value.params.id}`)
...
const submitAbout = async () => {
  try {
    ...
    if (image.value) {
      ...
      const imageResponse = await axios.post("http://localhost:3001/api/upload/single", formData, {
    ...
    const response = await axios.put(updateApi, updateData, {
      headers: {
