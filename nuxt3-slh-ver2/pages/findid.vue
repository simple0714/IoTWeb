...
const handleFindId = async (e) => {
  ...
  try {
    const response = await axios.get('/api/admin/findid', {
      params: {  // params 추가
        name: username.value,
        email: email.value
      },
    ...
    if (response.data) {
      foundId.value = response.data.ADMIN_ID
      modalMessage.value = `찾으시는 아이디는 ${foundId.value} 입니다.`
          ...
          <v-row class="fill-height">
            ...
            <v-col cols="12" md="6">
              <v-card-text class="topPadding">
                <v-form @submit.prevent="handleFindId" @keydown.enter="handleFindId">
                  <v-text-field
                    v-model="username"
                    label="이름"
