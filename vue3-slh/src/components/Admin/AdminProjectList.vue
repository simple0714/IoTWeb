...
import { useSelectedProjectId } from '../../stores/adminproject'  // Pinia store로 변환 필요
...
const URL = "http://localhost:3001/apis/project/listAll"
const URL_DELETE = "http://localhost:3001/apis/project/deleteProject"
...
const handleDelete = async (id) => {
  showConfirmDialog(
    ...
    async () => {
      try {
        dialog.value = false
        const response = await axios.delete(`${URL_DELETE}?projectNb=${id}`)
    ...
    <v-row>
      <v-col cols="12">
        ...
        <div class="search-area mb-4">
          <div class="search-container" @keydown.enter="handleSearch">
            <v-text-field
              v-model="searchTerm"
          ...
          <thead>
            <tr>
              ...
              <th class="text-center">
                <v-btn
                  color="black"
                  @click="props.onSelect('addproject')"
                >
                  추가
                </v-btn>
              </th>
